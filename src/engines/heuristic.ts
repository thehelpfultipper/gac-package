...
export async function generateHeuristic(
  ...
  const isArchitecturalChange =
    primaryFocus.focus === "engine-refactor" ||
  ...
  const docsSignificant =
    docsFiles.some((f) => /README\.md$/i.test(f.path)) &&
  ...
  const isMixed =
    docsFiles.length > 0 && codeFiles.length > 0 && docsSignificant;
  ...
  const selectedDesc =
    variant === 0
      ? primaryDesc
      : descriptions[Math.min(variant, descriptions.length - 1)];
  ...
  if (config.style === "plain" || config.style === "mix") {
    if (isMixed) {
      ...
      candidates.push(
        capitalizeFirst(`${selectedDesc.verb} ${selectedDesc.noun}`)
      );
    }
  }
  ...
  if (config.style === "gitmoji" || config.style === "mix") {
    ...
    if (isMixed) {
      ...
      const scopePart = scope ? `${scope}: ` : "";
      candidates.push(
        `${emoji} ${scopePart}${selectedDesc.verb} ${selectedDesc.noun}`
      );
    }
  }
  ...
  const usedDescs = new Set([
    variant === 0 ? 0 : Math.min(variant, descriptions.length - 1),
  ]);
  let altIndex = 1;
  ...
  while (candidates.length < 3 && altIndex < descriptions.length) {
    ...
    const alt =
      config.style === "gitmoji"
        ? `${getEmoji(type)} ${scope ? `${scope}: ` : ""}${altDesc.verb} ${
            altDesc.noun
          }`
...
function detectPrimaryFocus(
  ...
  const newEngines = files.filter(
    (f) =>
      f.status === "A" &&
      /\/engines\/[^/]+\.(ts|js)$/.test(f.path) &&
      !/heuristic\./.test(f.path)
  );
  const newShared = files.filter(
    (f) => f.status === "A" && /\/engines\/shared\.(ts|js)$/.test(f.path)
  );
  const modifiedEngines = files.filter(
    (f) =>
      f.status === "M" &&
      /\/engines\/[^/]+\.(ts|js)$/.test(f.path) &&
      !/heuristic\.|shared\./i.test(f.path)
  );
  const modifiedHeuristic = files.some(
    (f) => f.status === "M" && /\/engines\/heuristic\.(ts|js)$/.test(f.path)
  );

  // Pattern: New engine(s) + shared utilities + multiple modifications = ARCHITECTURE REFACTOR
  if (
    (newEngines.length > 0 || newShared.length > 0) &&
    modifiedEngines.length >= 2
  ) {
  ...
  if (
    newShared.length > 0 &&
    (modifiedEngines.length >= 2 || modifiedHeuristic)
  ) {
    return {
      focus: "engine-refactor",
...
function buildDescriptions(
  ...
  if (
    ...
    const hasConfigChanges = files.some((f) =>
      /config\.(ts|js|json)/.test(f.path)
    );
    const hasCLIChanges = files.some((f) => /cli\.(ts|js)/.test(f.path));
    const hasGeneratorChanges = files.some((f) =>
      /generator\.(ts|js)/.test(f.path)
    );
    ...
    if (supportingParts.length > 0) {
      ...
      const engineMatch = primaryFocus.detail.match(
        /\b([A-Z][a-z]+)\s+(engine|integration)/
      );
      const engineName = engineMatch ? engineMatch[1] : "engine";

      const supportingDesc =
        supportingParts.length === 1
          ? supportingParts[0]
          : supportingParts.length === 2
          ? supportingParts.join(" and ")
          : `${supportingParts.slice(0, -1).join(", ")}, and ${
              supportingParts[supportingParts.length - 1]
            }`;
  ...
  // Alternative 4: Category-based perspective (focus on top category for variety)
  if (categories.size >= 2 && descriptions.length < 4) {
    const topCatEntry = Array.from(categories.entries()).sort(
      (a, b) => b[1] - a[1]
    )[0];
    const noun = prettifyCategory(topCatEntry[0], files);

    const isDuplicate = descriptions.some(
      (d) => d.noun.toLowerCase() === noun.toLowerCase()
    );
    if (!isDuplicate) {
      descriptions.push({
        verb: getDefaultVerb(type, variant + descriptions.length),
        noun,
      });
    }
  }
  ...
  if (
    scope &&
    primaryFocus.focus !== "scope-specific" &&
    descriptions.length < 5
  ) {
    const topCat = Array.from(categories.entries()).sort(
      (a, b) => b[1] - a[1]
    )[0];
    if (topCat) {
...
function prettifyCategory(category: string, files: any[]): string {
  ...
  if (category === "dependencies") return "dependencies";
  return category;
}
...
function detectChangeType(
  ...
  const addedCode = files.some(
    (f) =>
      f.status === "A" && /\.(tsx?|jsx?|py|go|rs|java|cs|rb|php)$/.test(f.path)
  );
  const hasNewKeywords =
    /\bnew\s+(feature|component|function|class|endpoint|route|api)\b/i.test(
      diff
  ...
  const isRefactor =
    /\b(refactor|restructure|reorganize|cleanup|simplify)\b/i.test(diff) ||
    (files.every(
      (f) => f.status === "M" && f.additions > 0 && f.deletions > 0
    ) &&
      !addedCode);
  ...
  const isChore =
    files.some((f) =>
      /(package\.json|package-lock|yarn\.lock|Cargo\.toml|go\.mod)/.test(f.path)
    ) ||
    /\b(deps|dependency|dependencies|version|bump|upgrade|lockfile)\b/i.test(
      diff
    );
...
function detectScope(files: any[]): string {
  ...
  const domains: Record<string, RegExp> = {
    engine:
      /\b(engine|engines|heuristic|ollama|openai|gemini|anthropic|claude)\b/,
    ...
    ci: /\b(build|ci|pipeline|workflow|docker|vite|webpack)\b|\.github|\.gitlab/,
  ...
  const best = Object.keys(counts)
    .filter((k) => counts[k] > 0)
    .sort(
      (a, b) => counts[b] - counts[a] || order.indexOf(a) - order.indexOf(b)
    )[0];

  if (best) {
    const totalCodeWeight =
      paths.reduce((sum, p, i) => sum + (codeMask[i] ? weights[i] : 0), 0) ||
  ...
  const meaningful = commonParts.filter(
    (p: string) =>
      ![
        "src",
        "lib",
...
function extractTopics(diff: string, files?: any[]): string[] {
  ...
  const addIf = (re: RegExp, label: string) => {
    if (re.test(diff)) topics.add(label);
  };
...
function extractDocsTopic(diff: string): string {
  ...
  const addedLinks = lines.filter(
    (l) => /^\+/.test(l) && /https?:\/\//i.test(l)
  ).length;
  const removedLinks = lines.filter(
    (l) => /^-/.test(l) && /https?:\/\//i.test(l)
  ).length;
  if (addedLinks + removedLinks > 0) return "links";

  const smallTextOnly =
    lines.every((l) => !/^diff --git /.test(l)) &&
    lines.filter((l) => /^[+-]/.test(l)).length < 10;
...
function buildSubject(
  ...
  if (categories.size === 1) {
    ...
    return includeVerb
      ? `${action} ${noun} across ${files.length} files`
      : `${noun} across ${files.length} files`;
  }
  ...
  if (topCategories.length === 2) {
    const engineNoun = topCategories.includes("engine")
      ? detectEngineNoun(files)
      : "";
    const first =
      topCategories[0] === "engine"
        ? engineNoun || "engines"
        : topCategories[0];
    const second =
      topCategories[1] === "engine"
        ? engineNoun || "engines"
        : topCategories[1];
    return includeVerb
...
function categorizeFiles(files: any[]): Map<string, number> {
  ...
  for (const file of files) {
    ...
    // Weigh new files MORE heavily (architectural significance)
  ...
  // If other more specific categories exist, remove the generic 'files' fallback.
  if (categories.size > 1 && categories.has("files")) {
    categories.delete("files");
  }
...
function generateAlternative(
  ...
  const pool = byType[type] || [
    `update ${target} with ${files.length} change${
      files.length > 1 ? "s" : ""
    }`,
    `improve ${target}`,
  ...
  const salt = (type + target + (index ?? 0))
    .split("")
